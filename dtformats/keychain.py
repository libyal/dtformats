...
class KeychainDatabaseFile(data_format.BinaryDataFile):
        ...
        if table.columns:
            ...
            for index, column in enumerate(table.columns):
                if self._debug:
                    ...
                    self._DebugPrintData(
                        description,
                        record_data[attribute_value_offset:attribute_value_end_offset],
                    )
    ...
    def _ReadRecordSchemaIndexes(self, tables, file_object, record_offset):
        ...
        record_values, _ = self._ReadStructureFromFileObject(
            file_object, file_offset, data_type_map, "schema indexes record values"
        )
        ...
        if self._debug:
            file_offset = file_object.tell()
            trailing_data_size = record_header.data_size - (file_offset - record_offset)
    ...
    def _ReadRecordSchemaInformation(self, tables, file_object, record_offset):
        ...
        record_values, _ = self._ReadStructureFromFileObject(
            file_object, file_offset, data_type_map, "schema information record values"
        )
