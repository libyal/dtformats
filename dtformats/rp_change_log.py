...
class RestorePointChangeLogFile(data_format.BinaryDataFile):
    ...
    def _DebugPrintChangeLogEntryRecord(self, change_log_entry_record):
        ...
        self._DebugPrintValue("Signature", f"0x{change_log_entry_record.signature:08x}")
        ...
        self._DebugPrintValue("Unknown2", f"0x{change_log_entry_record.unknown2:08x}")
    ...
    def _DebugPrintRecordHeader(self, record_header):
        ...
        record_type_string = (
            data_type_map.GetName(record_header.record_type) or "UNKNOWN"
        )

        self._DebugPrintValue(
            "Record type", f"{record_header.record_type:d} ({record_type_string:s})"
        )
    ...
    def _ReadFileHeader(self, file_object):
        ...
        try:
            copy_of_record_size = self._ReadStructureFromByteStream(
                record_data[-4:], file_offset, data_type_map, "copy of record size"
            )
        except (ValueError, errors.ParseError) as exception:
            raise errors.ParseError(
                f"Unable to parse copy of record size with error: {exception!s}"
