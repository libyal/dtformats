...
class AutomaticDestinationsFile(data_format.BinaryDataFile):
    ...
    def GetJumpListEntries(self):
        ...
        for olecf_item in iter(self._olecf_file.root_item.sub_items):
...
class CustomDestinationsFile(data_format.BinaryDataFile):
    ...
    _DEBUG_INFORMATION = data_format.BinaryDataFile.ReadDebugInformationFile(
        "jump_list.debug.yaml",
        custom_format_callbacks={"category_type": "_FormatIntegerAsCategoryType"},
    )
    ...
    _CATEGORY_TYPES = {0: "Custom category", 1: "Known category", 2: "User tasks"}
    ...
    def _ReadCategoryHeader(self, file_object, file_offset):
        ...
        if category_header.category_type == 0:
            ...
            setattr(
                category_header, "category_name", category_header_value.category_name
            )
    ...
    def _ReadJumpListEntry(self, file_object):
        ...
        file_offset = file_object.tell()
        if self._debug:
            self._DebugPrintText(f"Reading LNK file at offset: 0x{file_offset:08x}\n")
