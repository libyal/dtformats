...
class BinaryCookiesFile(data_format.BinaryDataFile):
    ...
    def _ReadPage(self, file_object, file_offset, page_size):
        ...
        """
        page_data = self._ReadData(file_object, file_offset, page_size, "page data")
        ...
        if self._debug:
            self._DebugPrintData("Page header data", page_data[:page_header_data_size])
    ...
    def _ReadRecord(self, page_data, record_offset):
        ...
        try:
            record_header = self._ReadStructureFromByteStream(
                page_data[record_offset:], record_offset, data_type_map, "record header"
            )
        except (ValueError, errors.ParseError) as exception:
            raise errors.ParseError(
