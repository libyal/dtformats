    ...
    def Build(self, class_definitions):
        ...
        for class_definition in class_definitions:
            for name, property_definition in class_definition.properties.items():
                ...
                if (
                    largest_offset is None
                    or largest_offset < property_definition.value_data_offset
                ):
...
class IndexBinaryTreeFile(data_format.BinaryDataFile):
    ...
    _DEBUG_INFORMATION = data_format.BinaryDataFile.ReadDebugInformationFile(
        "wmi_repository.debug.yaml",
        custom_format_callbacks={"page_type": "_FormatIntegerAsPageType"},
    )
    ...
    def _DebugPrintPageBody(self, page_body):
        ...
        self._DebugPrintValue(
            "Key data size", f"{page_body.key_data_size:d} ({number_of_bytes:d} bytes)"
        )
        self._DebugPrintData("Key data", page_body.key_data)

        self._DebugPrintDecimalValue("Number of values", page_body.number_of_values)
    ...
    def _ReadPage(self, file_object, file_offset):
        ...
        if page_header.page_type == 0xACCC:
            ...
            page_body = self._ReadStructureFromByteStream(
                page_body_data, file_offset, data_type_map, "page body", context=context
            )
    ...
    def _ReadPageKeyData(self, index_binary_tree_page, page_body):
        ...
        for page_key_index, key_offset in enumerate(page_body.key_offsets):
            ...
            page_key = self._ReadStructureFromByteStream(
                key_data[page_key_offset:],
                page_key_offset,
                data_type_map,
                f"page key: {page_key_index:d}",
                context=context,
            )
            if self._debug:
...
class MappingFile(data_format.BinaryDataFile):
    ...
    _DEBUG_INFORMATION = data_format.BinaryDataFile.ReadDebugInformationFile(
        "wmi_repository.debug.yaml",
        custom_format_callbacks={"page_number": "_FormatIntegerAsPageNumber"},
    )
...
class ObjectsDataFile(data_format.BinaryDataFile):
    ...
    _DEBUG_INFORMATION = data_format.BinaryDataFile.ReadDebugInformationFile(
        "wmi_repository.debug.yaml",
        custom_format_callbacks={"offset": "_FormatIntegerAsOffset"},
    )
...
class RepositoryFile(data_format.BinaryDataFile):
    ...
    _DEBUG_INFO_INSTANCE_ROOT_NODE = [
        (
            "child_objects_root_node_offset",
            "Child objects root node offset",
            "_FormatIntegerAsOffset",
        ),
        ("name_node_offset", "Name node offset", "_FormatIntegerAsOffset"),
        (
            "instance_branch_node_offset",
            "Instance branch node offset",
            "_FormatIntegerAsOffset",
        ),
        ("unknown1", "Unknown1", "_FormatIntegerAsDecimal"),
        (
            "child_objects_list_node_offset",
            "Child objects list node offset",
            "_FormatIntegerAsOffset",
        ),
        ("unknown2", "Unknown2", "_FormatIntegerAsOffset"),
        ("unknown_node5_offset", "Unknown node 5 offset", "_FormatIntegerAsOffset"),
    ...
    def _ReadChildObjectsListElementNode(self, block_data, file_offset):
        ...
        data_type_map = self._GetDataTypeMap("cim_rep_child_objects_list_element_node")

        list_element_node = self._ReadStructureFromByteStream(
            block_data, file_offset, data_type_map, "child objects list element node"
        )
        if self._debug:
            debug_info = self._DEBUG_INFORMATION.get(
    ...
    def _ReadChildObjectsTree(self, file_object, root_node_offset):
        ...
        root_node = self._ReadChildObjectsTreeRootNode(node_cell.data, root_node_offset)
    ...
    def _ReadInstanceLeafNode(self, block_data, file_offset):
        ...
        if self._debug:
            debug_info = self._DEBUG_INFORMATION.get("cim_rep_instance_leaf_node", None)
    ...
    def _ReadInstanceRootNode(self, block_data, file_offset):
        ...
        if self._debug:
            debug_info = self._DEBUG_INFORMATION.get("cim_rep_instance_root_node", None)
    ...
    def _ReadClassDefinition(self, file_object, branch_node_offset):
        ...
        leaf_node = self._ReadClassDefinitionLeafNode(node_cell.data, leaf_node_offset)

        class_definition = ClassDefinition(
            debug=self._debug, output_writer=self._output_writer
        )
        class_definition.ReadClassDefinitionBlock(
            leaf_node.class_definition_block_data, record_data_offset=leaf_node_offset
        )
    ...
    def _ReadClassDefinitionHierarchy(self, file_object, root_node_offset):
        ...
        """
        node_cell = self._ReadNodeCell(file_object, root_node_offset - 4)
        root_node = self._ReadClassDefinitionRootNode(node_cell.data, root_node_offset)
        ...
        if self._debug:
            if root_node.child_objects_root_node_offset > 40:
                for value_node_offset in self._ReadChildObjectsTree(
                    ...
                    if value_node_offset > 40:
                        node_cell = self._ReadNodeCell(
                            file_object, value_node_offset - 4
                        )
    ...
    def _ReadInstance(self, file_object, branch_node_offset):
        ...
        instance.ReadInstanceBlockData(
            class_value_data_map,
            leaf_node.instance_block_data,
    ...
    def _ReadInstanceHierarchy(self, file_object, root_node_offset):
        ...
        if (
            root_node.child_objects_root_node_offset > 40
            and root_node.child_objects_root_node_offset != 0xFFFFFFFF
        ):
            for value_node_offset in self._ReadChildObjectsTree(
                file_object, root_node.child_objects_root_node_offset
            ):
                if value_node_offset > 40:
                    ...
                    if self._debug:
                        if instance_leaf_value_node.name_node_offset > 40:
                            node_cell = self._ReadNodeCell(
                                file_object,
                                instance_leaf_value_node.name_node_offset - 4,
                            )
                            self._ReadNameNode(
                                node_cell.data,
                                instance_leaf_value_node.name_node_offset,
                            )
        ...
        if self._debug:
            if root_node.child_objects_list_node_offset > 40:
                for value_node_offset in self._ReadChildObjectsList(
                    ...
                    if value_node_offset > 40:
                        node_cell = self._ReadNodeCell(
                            file_object, root_node_offset - 4
                        )
                        root_node = self._ReadClassDefinitionRootNode(
                        ...
                        if root_node.class_definition_branch_node_offset > 40:
                            self._ReadClassDefinition(
                                file_object,
                                root_node.class_definition_branch_node_offset,
                            )
    ...
    def ReadFileObject(self, file_object):
        ...
        while file_offset < file_size:
            ...
            if (
                not self._debug
                and self._root_namespace_node_offset is not None
                and self._system_class_definition_root_node_offset is not None
            ):
                break
...
class CIMObject(data_format.BinaryDataFormat):
    ...
    def _FormatIntegerAsDataType(self, integer):
        ...
        """
        data_type_string = self._CIM_DATA_TYPES.GetName(integer & 0x3FFF) or "UNKNOWN"
    ...
    def _ReadCIMString(
        ...
        cim_string = self._ReadStructureFromByteStream(
            record_data[string_offset:],
            record_data_offset + string_offset,
            data_type_map,
            description,
        )
        if self._debug:
...
class ClassDefinition(CIMObject):
    ...
    _DEBUG_INFO_CLASS_DEFINITION_BLOCK = [
        ...
        (
            "super_class_name_block_size",
            "Super class name block size",
            "_FormatIntegerAsDecimal",
        ),
        (
            "super_class_name_block_data",
            "Super class name block data",
            "_FormatDataInHexadecimal",
        ),
        ("qualifiers_block_size", "Qualifiers block size", "_FormatIntegerAsDecimal"),
        ("qualifiers_block_data", "Qualifiers block data", "_FormatDataInHexadecimal"),
        (
            "number_of_property_descriptors",
            "Number of property descriptors",
            "_FormatIntegerAsDecimal",
        ),
        (
        ...
        ("values_data_size", "Values data size", "_FormatIntegerAsPropertiesBlockSize"),
    ...
    def _ReadQualifiers(
        self, qualifiers_data, qualifiers_data_offset, values_data, values_data_offset
    ):
        """Reads qualifiers.
        ...
        while qualifiers_data_offset < len(qualifiers_data):
            ...
            qualifier_descriptor = self._ReadStructureFromByteStream(
                qualifiers_data[qualifiers_data_offset:],
                record_data_offset,
                data_type_map,
            ...
            qualifier_name = self._ReadQualifierName(
                qualifier_index,
                qualifier_descriptor.name_offset,
                values_data,
                values_data_offset,
            )
            ...
            if cim_data_type == "CIM-TYPE-BOOLEAN":
                ...
                qualifier_value = self._ReadCIMString(
                    qualifier_descriptor.value_offset,
                    values_data,
                    values_data_offset,
    ...
    def DebugPrint(self):
        ...
        for property_name, class_definition_property in self.properties.items():
            ...
            for (
                qualifier_name,
                qualifier_value,
            ) in class_definition_property.qualifiers.items():
                self._DebugPrintValue(
                    f"        Qualifier: {qualifier_name:s}", f"{qualifier_value!s}"
                )
    ...
    def ReadClassDefinitionBlock(self, class_definition_data, record_data_offset=0):
        ...
        class_name = self._ReadCIMString(
            ...
            class_definition_block.values_data,
            value_data_offset,
            "class name",
        )
        ...
        if class_definition_block.super_class_name_block_size > 4:
            super_class_name = self._ReadCIMString(
                0,
                class_definition_block.super_class_name_block_data,
                super_class_name_block_offset,
                "super class name",
            )
        ...
        if class_definition_block.qualifiers_block_size > 4:
            class_qualifiers = self._ReadQualifiers(
                class_definition_block.qualifiers_block_data,
                qualifiers_block_offset,
                class_definition_block.values_data,
                value_data_offset,
            )
...
class ClassDefinitionReference(CIMObject):
    ...
    _DEBUG_INFO_CLASS_DEFINITION_OBJECT_RECORD = [
        ("super_class_name_size", "Super class name size", "_FormatIntegerAsDecimal"),
        ("super_class_name", "Super class name", "_FormatString"),
        ("date_time", "Unknown date and time", "_FormatIntegerAsFiletime"),
        (
            "class_definition_block_size",
            "Class definition block size",
            "_FormatIntegerAsDecimal",
        ),
        (
            "class_definition_block_data",
...
class InstanceReference(CIMObject):
    ...
    _DEBUG_INFO_INSTANCE_OBJECT_RECORD = [
        ...
        ("instance_block_data", "Instance block data", "_FormatDataInHexadecimal"),
    ]
...
class Instance(CIMObject):
    ...
    _DEBUG_INFO_INSTANCE_BLOCK = [
        ...
        ("unknown1", "Unknown1", "_FormatIntegerAsHexadecimal2"),
        ("property_state_bits", "Property state bits", "_FormatDataInHexadecimal"),
        ("property_values_data", "Property values data", "_FormatDataInHexadecimal"),
        ("qualifiers_block_size", "Qualifiers block size", "_FormatIntegerAsDecimal"),
        ("qualifiers_block_data", "Qualifiers block data", "_FormatDataInHexadecimal"),
        ("dynamic_block_type", "Dynamic block type", "_FormatIntegerAsDecimal"),
        (
            "dynamic_block_value1",
            "Dynamic block value1",
    ...
    def ReadInstanceBlockData(
        ...
        if instance_block.dynamic_block_type == 2:
            data_type_map = self._GetDataTypeMap("instance_block_dynamic_type2_header")

            dynamic_type2_header = self._ReadStructureFromByteStream(
                instance_data[data_offset:],
        ...
        self.class_name = self._ReadCIMString(
            instance_block.class_name_offset, values_data, data_offset, "class name"
        )
        ...
        for property_value_data_map in class_value_data_map.properties.values():
            ...
            if property_value_data_map.data_type in self._FIXED_SIZE_VALUE_DATA_TYPES:
                ...
                property_value = self._ReadStructureFromByteStream(
                    ...
                    property_values_data_offset + property_map_offset,
                    data_type_map,
                    description,
                )
                if self._debug:
                ...
                if string_array_offset > 0:
                    ...
                    string_array = self._ReadStructureFromByteStream(
                        ...
                        data_offset + string_array_offset,
                        data_type_map,
                        description,
                    )
                    property_value = []
...
class Registration(CIMObject):
    ...
    _DEBUG_INFO_REGISTRATION_OBJECT_RECORD = [
        ("name_space_string_size", "Name space string size", "_FormatIntegerAsDecimal"),
        ("name_space_string", "Name space string", "_FormatString"),
        ("class_name_string_size", "Class name string size", "_FormatIntegerAsDecimal"),
        ("class_name_string", "Class name string", "_FormatString"),
        (
            "instance_name_string_size",
            "Instance name string size",
            "_FormatIntegerAsDecimal",
        ),
        ("instance_name_string", "Instance name string", "_FormatString"),
        ("index_key_string_size", "Index key string size", "_FormatIntegerAsDecimal"),
    ...
    def ReadObjectRecord(self, object_record_data):
        ...
        if self._debug:
            self._DebugPrintStructureObject(
                registration_object_record, self._DEBUG_INFO_REGISTRATION_OBJECT_RECORD
            )
...
class CIMRepository(data_format.BinaryDataFormat):
    ...
    def _GetActiveMappingFile(self, path):
        ...
        if (
            mapping_ver_file_number is not None
            and mapping_ver_file_number != active_mapping_file_number
        ):
    ...
    def _GetClassValueMapByHash(self, class_name_hash):
        ...
        class_value_data_map = self._class_value_data_map_by_hash.get(lookup_key, None)
    ...
    def _GetIndexPageByMappedPageNumber(self, mapped_page_number):
        ...
        if not index_page:
            logging.warning(f"Unable to read index binary-tree page: {page_number:d}.")
            return None
    ...
    def _OpenMappingFile(self, path, filename):
        ...
        mapping_file = MappingFile(debug=self._debug, output_writer=self._output_writer)
    ...
    def _ReadInstanceFromObjectRecord(self, object_record):
        ...
        instance_reference = InstanceReference(
            self.format_version, debug=self._debug, output_writer=self._output_writer
        )
