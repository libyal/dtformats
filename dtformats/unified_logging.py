...
class OpenDirectoryMembershipDetailsFormatStringDecoder(
    ...
    # Using a class constant significantly speeds up the time required to load
    # the dtFabric definition file.
    _FABRIC = data_format.BinaryDataFile.ReadDefinitionFile("macos_open_directory.yaml")
...
class TimesyncDatabaseFile(data_format.BinaryDataFile):
    ...
    _DEBUG_INFORMATION = data_format.BinaryDataFile.ReadDebugInformationFile(
        "aul_timesync.debug.yaml",
        custom_format_callbacks={
...
class TraceV3File(data_format.BinaryDataFile):
        ...
        if strings_file_type == 0x0008:
            ...
            for uuid_entry in process_information_entry.uuid_entries:
                ...
                if load_address_lower <= (
                    ...
                    if self._catalog:
                        strings_file_identifier = self._catalog.uuids[
                            uuid_entry.uuid_index
                        ]
    ...
    def _GetSubSystemStrings(self, process_information_entry, sub_system_identifier):
        ...
        if self._debug and sub_system_identifier is not None:
            self._DebugPrintDecimalValue("Sub system identifier", sub_system_identifier)
        ...
        while data_offset < lz4_block_header.uncompressed_data_size:
            ...
            if self._debug:
                self._DebugPrintData(
                    "Chunk header data",
                    uncompressed_data[data_offset : data_offset + 16],
                )

            chunkset_chunk_header = self._ReadStructureFromByteStream(
                uncompressed_data[data_offset:],
                data_offset,
                data_type_map,
                "chunk header",
            )
    ...
    def _ReadFirehoseChunkData(
        ...
        if self._debug:
            ...
            self._GetTimestamp(
                firehose_header.base_continuous_time, description="Base continuous time"
            )
    ...
    def _ReadFirehoseTracepointActivityData(
        ...
        activity = self._ReadStructureFromByteStream(
            tracepoint_data, data_offset, data_type_map, "activity", context=context
        )
        if self._debug:
            debug_info = self._DEBUG_INFORMATION.get(
    ...
    def _ReadFirehoseTracepointSignpostData(self, flags, tracepoint_data, data_offset):
        ...
        signpost = self._ReadStructureFromByteStream(
            tracepoint_data, data_offset, data_type_map, "signpost", context=context
        )
        if self._debug:
            debug_info = self._DEBUG_INFORMATION.get(
    ...
    def _ReadHeaderChunk(self, file_object, file_offset):
        ...
        if self._debug:
            ...
            self._DebugPrintStructureObject(header_chunk.system_information, debug_info)
    ...
    def _ReadOversizeChunkData(self, chunk_data, chunk_data_size, data_offset):
        ...
        oversize_chunk = self._ReadStructureFromByteStream(
            chunk_data, data_offset, data_type_map, "Oversize chunk", context=context
        )
        if self._debug:
        ...
        if self._debug and data_size < chunk_data_size:
            self._DebugPrintData("Trailing Oversize chunk data", chunk_data[data_size:])
    ...
    def _ReadSimpleDumpChunkData(self, chunk_data, chunk_data_size, data_offset):
        ...
        simpledump_chunk = self._ReadStructureFromByteStream(
            chunk_data, data_offset, data_type_map, "SimpleDump chunk", context=context
        )
        if self._debug:
    ...
    def _ReadStateDumpChunkData(self, chunk_data, chunk_data_size, data_offset):
        ...
        statedump_chunk = self._ReadStructureFromByteStream(
            chunk_data, data_offset, data_type_map, "StateDump chunk", context=context
        )
        if self._debug:
