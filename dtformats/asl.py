...
class AppleSystemLogFile(data_format.BinaryDataFile):
    ...
    def _ReadRecord(self, file_object, file_offset):
        ...
        record_strings_data = self._ReadData(
            file_object,
            record_strings_data_offset,
            record_strings_data_size,
            "record strings data",
        )
        if self._debug:
        ...
        hostname = self._ReadRecordString(file_object, record.hostname_string_offset)

        sender = self._ReadRecordString(file_object, record.sender_string_offset)

        facility = self._ReadRecordString(file_object, record.facility_string_offset)

        message = self._ReadRecordString(file_object, record.message_string_offset)
        ...
        additional_data = self._ReadData(
            file_object, file_offset, additional_data_size, "record additional data"
        )
        if self._debug:
