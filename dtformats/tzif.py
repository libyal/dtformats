...
class TimeZoneInformationFile(data_format.BinaryDataFile):
    ...
    def _DebugPrintFileHeader(self, file_header):
        ...
        self._DebugPrintValue("Format version", f"0x{file_header.format_version:02x}")
        ...
        self._DebugPrintValue(
            "Number of transition times", f"{file_header.number_of_transition_times:d}"
        )

        self._DebugPrintValue(
            "Number of local time types", f"{file_header.number_of_local_time_types:d}"
        )
    ...
    def _DebugPrintTransitionTimes(self, transition_times):
        ...
        for index, transition_time in enumerate(transition_times):
            self._DebugPrintValue(f"Transition time: {index:d}", f"{transition_time:d}")
    ...
    def _ReadTransitionTimeIndex(self, file_object, file_header):
        ...
        context = dtfabric_data_maps.DataTypeMapContext(
            values={"tzif_file_header": file_header}
        )
