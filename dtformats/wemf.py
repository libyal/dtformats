...
class EMFFile(data_format.BinaryDataFile):
    ...
    def _DebugPrintFileHeader(self, file_header):
        ...
        """
        record_type_string = (
            self._RECORD_TYPE.GetName(file_header.record_type) or "UNKNOWN"
        )
        self._DebugPrintValue(
            "Record type", (f"0x{file_header.record_type:04x} ({record_type_string:s})")
        )
        ...
        self._DebugPrintValue("Format version", f"0x{file_header.format_version:04x}")

        self._DebugPrintValue("File size", f"{file_header.file_size:d}")

        self._DebugPrintValue("Number of records", f"{file_header.number_of_records:d}")

        self._DebugPrintValue("Number of handles", f"{file_header.number_of_handles:d}")
    ...
    def _DebugPrintRecordHeader(self, record_header):
        ...
        """
        record_type_string = (
            self._RECORD_TYPE.GetName(record_header.record_type) or "UNKNOWN"
        )
        self._DebugPrintValue(
            "Record type",
    ...
    def _ReadRecord(self, file_object, file_offset):
        ...
        if self._debug:
            self._ReadRecordData(file_object, record_header.record_type, data_size)

        return Record(
            record_header.record_type, record_header.record_size, data_offset, data_size
        )
...
class WMFFile(data_format.BinaryDataFile):
    ...
    def _DebugPrintHeader(self, file_header):
        ...
        self._DebugPrintValue(
            "Maximum number of object", f"{file_header.maximum_number_of_objects:d}"
        )
        self._DebugPrintValue(
    ...
    def _DebugPrintPlaceable(self, placeable):
        ...
        self._DebugPrintValue("Resource handle", f"0x{placeable.resource_handle:04x}")
    ...
    def _ReadHeader(self, file_object):
        ...
        if header.record_size != 9:
            raise errors.ParseError(f"Unsupported record size: {header.record_size:d}")
    ...
    def _ReadRecord(self, file_object, file_offset):
        ...
        return Record(record_header.record_type, record_size, data_offset, data_size)
    ...
    def _ReadRecordData(self, file_object, record_type, data_size):
        ...
        if self._debug:
            if record_type == 0x0103:
                ...
                stretch_mode_string = (
                    self._MAP_MODE.GetName(record.stretch_mode) or "UNKNOWN"
                )
                self._DebugPrintValue(
                    "Stretch mode",
