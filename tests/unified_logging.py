...
class StringFormatterTest(test_lib.BaseTestCase):
    ...
    def testParseFormatString(self):
        ...
        test_formatter.ParseFormatString("%{signpost.description:attribute,public}llu")
        self.assertEqual(test_formatter._decoders, [["signpost.description:attribute"]])
        self.assertEqual(test_formatter._format_string, "{0:s}")
        self.assertEqual(len(test_formatter._operators), 1)
        ...
        self.assertEqual(
            test_formatter._decoders, [["mask.hash", "mdnsresponder:ip_addr"]]
        )
        self.assertEqual(test_formatter._format_string, "{0:s}")
        self.assertEqual(len(test_formatter._operators), 1)
...
class LocationClientAuthorizationStatusFormatStringDecoder(test_lib.BaseTestCase):
...
class LocationClientManagerStateFormatStringDecoderTest(test_lib.BaseTestCase):
    ...
    def testFormatValue(self):
        """Tests the FormatValue function."""
        test_decoder = unified_logging.LocationClientManagerStateFormatStringDecoder()
...
class LocationLocationManagerStateFormatStringDecoderTest(test_lib.BaseTestCase):
...
class MDNSDNSIdentifierAndFlagsFormatStringDecoder(test_lib.BaseTestCase):
    ...
    def testFormatValue(self):
        """Tests the FormatValue function."""
        test_decoder = unified_logging.MDNSDNSIdentifierAndFlagsFormatStringDecoder()
...
class OpenDirectoryMembershipDetailsFormatStringDecoderTest(test_lib.BaseTestCase):
...
class OpenDirectoryMembershipTypeFormatStringDecoderTest(test_lib.BaseTestCase):
    ...
    def testFormatValue(self):
        """Tests the FormatValue function."""
        test_decoder = unified_logging.OpenDirectoryMembershipTypeFormatStringDecoder()
...
class SignpostDescriptionAttributeFormatStringDecoderTest(test_lib.BaseTestCase):
    """Signpost description attribute value format string decoder tests."""

    def testFormatValue(self):
        """Tests the FormatValue function."""
        test_decoder = unified_logging.SignpostDescriptionAttributeFormatStringDecoder()
...
class WindowsNTSecurityIdentifierFormatStringDecoderTest(test_lib.BaseTestCase):
    ...
    def testFormatValue(self):
        """Tests the FormatValue function."""
        test_decoder = unified_logging.WindowsNTSecurityIdentifierFormatStringDecoder()
...
class DSCFileTest(test_lib.BaseTestCase):
    ...
    def testReadUUIDDescriptors(self):
        ...
        expected_path = "/System/Library/Extensions/AppleD2207PMU.kext/AppleD2207PMU"
...
class TimesyncDatabaseFileTest(test_lib.BaseTestCase):
    ...
    def testReadFileRecord(self):
        ...
        output_writer = test_lib.TestOutputWriter()
        test_file = unified_logging.TimesyncDatabaseFile(output_writer=output_writer)
...
class TraceV3FileTest(test_lib.BaseTestCase):
    ...
    _FIREHOSE_TRACEPOINT_ACTIVITY_DATA = bytes(
        bytearray(
            [
                0xE0,
                0x00,
    ...
    _FIREHOSE_TRACEPOINT_BACKTRACE_DATA = bytes(
        bytearray(
            [
                0x01,
                0x00,
    ...
    _FIREHOSE_TRACEPOINT_LOG_DATA = bytes(
        bytearray(
            [
                0x6C,
                0x86,
    ...
    _FIREHOSE_TRACEPOINT_LOSS_DATA = bytes(
        bytearray(
            [
                0xCE,
                0x9A,
    ...
    _FIREHOSE_TRACEPOINT_TRACE_DATA1 = bytes(bytearray([0x2B, 0xF4, 0x03, 0x00, 0x00]))

    _FIREHOSE_TRACEPOINT_TRACE_DATA2 = bytes(
        bytearray(
            [
                0xF6,
                0x21,
    ...
    def testCalculateProgramCounter(self):
        ...
        tracepoint_data_object = tracepoint_data_object_tuple(
            large_offset_data=0x0001,
            large_shared_cache_data=0x0002,
    ...
    def testReadBacktraceData(self):
        ...
        self.assertEqual(
            backtrace_frames[0].image_identifier,
            uuid.UUID("4be71104-3c12-38bd-b2f8-2c1ed6a3fd8d"),
        )
    ...
    def testReadOversizeChunkData(self):
        ...
        oversize_chunk = test_file._ReadOversizeChunkData(
            self._OVERSIZE_CHUNK_DATA, len(self._OVERSIZE_CHUNK_DATA), 0
        )
    ...
    def testReadSimpleDumpChunkData(self):
        ...
        test_file = unified_logging.TraceV3File(
            error_on_warning=False, output_writer=output_writer
        )

        log_entries = list(
            test_file._ReadSimpleDumpChunkData(
        ...
        self.assertEqual(
            log_entry.event_message, ("Skipping boot-task: restore-datapartition")
        )
        ...
        self.assertEqual(
            ...
            uuid.UUID("1d625353-a9fa-3ec9-94c0-b2952315b2d2"),
        )
    ...
    def testReadStateDumpChunkData(self):
        ...
        log_entries = list(
            test_file._ReadStateDumpChunkData(
        ...
        self.assertEqual(
            log_entry.sender_image_identifier,
            uuid.UUID("081b5e9e-59ea-39cd-83c9-eedb68a84076"),
        )
        self.assertIsNone(log_entry.sender_image_path)
        # Not converted due to test without catalog.
