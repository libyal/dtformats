...
class YAMLDebugDefinitionsFileTest(shared_test_lib.BaseTestCase):
    ...
    def testReadDefinition(self):
        ...
        test_definitions_file = yaml_definitions_file.YAMLDebugDefinitionsFile()

        debug_definition = test_definitions_file._ReadDefinition(self._FORMATTERS_YAML)
        ...
        debug_definition_attribute = debug_definition.attributes.get("signature", None)
        ...
        with self.assertRaises(errors.ParseError):
            test_definitions_file._ReadDefinition(
                {"data_type_map": "test_data_type_map", "attributes": []}
            )
            ...
            test_definitions_file._ReadDefinition(
                {
                    "data_type_map": "test_data_type_map",
                    "attributes": [
                        {"description": "Signature", "format": "custom:signature"}
                    ],
                }
            )
