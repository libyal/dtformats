...
class NSKeyedArchiverDecoder:
    ...
    def _DecodeNSArray(self, encoded_object, objects_list):
        ...
        return [
            self._DecodeObject(element, objects_list)
    ...
    def _DecodeObject(self, encoded_object, objects_list):
        ...
        if encoded_object is None or isinstance(encoded_object, (bool, int, float)):
        ...
        if isinstance(encoded_object, list):
            return [
                self._DecodeObject(element, objects_list) for element in encoded_object
            ]

        encoded_object_type = type(encoded_object)
        raise RuntimeError(f"Unsupported encoded object type: {encoded_object_type!s}")
    ...
    def _GetPlistUID(self, encoded_object):
        ...
        if (
            isinstance(encoded_object, dict)
            and "CF$UID" in encoded_object
            and len(encoded_object) == 1
        ):
