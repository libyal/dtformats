...
def Main():
    ...
    argument_parser = argparse.ArgumentParser(
        description=("Extracts information from a Windows Job file.")
    )
    ...
    argument_parser.add_argument(
        ...
        metavar="PATH",
        default=None,
        help=("path of the Windows Job file."),
    )
    ...
    output_writer.WriteText(f"\tIdentifier\t\t: {task_configuration.identifier:s}\n")
    output_writer.WriteText(
        f"\tApplication name\t: {task_configuration.application_name:s}\n"
    )
    output_writer.WriteText(f"\tParameters\t\t: {task_configuration.parameters:s}\n")

    output_writer.WriteText(f"\tAuthor\t\t\t: {task_configuration.author:s}\n")
    output_writer.WriteText(f"\tComment\t\t\t: {task_configuration.comment:s}\n")
