...
def Main():
    ...
    argument_parser = argparse.ArgumentParser(
        description=("Extracts information from MacOS keychain database files.")
    )
        ...
        for table in keychain_file.tables:
            ...
            for index, column in enumerate(table.columns):
                ...
                attribute_data_type = ATTRIBUTE_DATA_TYPES.get(
                    column.attribute_data_type, f"0x{column.attribute_data_type:08x}"
                )

                attribute_name = column.attribute_name or "NULL"
                print(
                    (
                        f"\t{index:d}\t{attribute_identifier:s}\t{attribute_name:s}\t"
