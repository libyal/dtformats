...
then
	...
	for FILE in `find setup.py config dtformats scripts tests -name \*.py`;
	do
		echo "Checking: ${FILE}";
...
then
	...
	if test -n "${TOXENV}";
	then
		docker exec ${CONTAINER_NAME} sh -c "export LANG=en_US.UTF-8; cd dtformats && tox -e ${TOXENV}";
	...
	then
		...
		docker exec ${CONTAINER_NAME} sh -c "export LANG=en_US.UTF-8; cd dtformats && python3 run_tests.py";
	fi
